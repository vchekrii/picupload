...
<body>
	<div id=wrap>
					...
					<tr>
						<c:choose>
							<c:when test="${users.banned=='true'}">
								<td><a href="user/${users.id}" class="banned"><spring:message code="label.user" />${users.ip}</a></td>
								<td><a href="unbanUser/admin/null/${users.id}"><spring:message code="label.unbanUser" /></a></td>
							</c:when>
							<c:otherwise>
								<td><a href="user/${users.id}"><spring:message code="label.user" />${users.ip}</a></td>
								<td><a href="banUser/admin/null/${users.id}"><spring:message code="label.banUser" /></a></td>
							</c:otherwise>
						</c:choose>
						<td><a href="deleteUser/admin/null/${users.id}"><spring:message code="label.deleteUser" /></a></td>
					</tr>
				</c:forEach>
			</table>
		...
		<!-- Images list -->
		<spring:message code="label.click" var="click"/>
		<c:forEach items="${allPictures}" var="pictures">
			<a href="#" rel="shareit${pictures.pic}"><img class="border" title="${click}" src="resources/uploadedImages/${pictures.pic106} /"></a>	
						...
						<ul>
							...
							<c:choose>
								<c:when test="${pictures.banned=='true'}">
									<li><a href="unbanImage/admin/null/${pictures.pic}"><spring:message code="label.unbanImage" /></a></li>
								</c:when>	
								<c:otherwise>
									<li><a href="banImage/admin/null/${pictures.pic}"><spring:message code="label.banImage" /></a></li>
								</c:otherwise>
							</c:choose>
							<li><a href="image/delete/admin/${pictures.pic}.jpg"><spring:message code="label.deleteImage" /></a></li>
			...
			<script type="text/javascript">
				$(document).ready(function() {
					$('a[rel="shareit<c:out value="${pictures.pic}"/>"]').click(function() {
						var img = $(this);
						var height = img.height();
						var top = img.offset().top-17;
						var left = img.offset().left + (img.width() /2) - ($('<c:out value="${pictures.pic}"/>').width() / 2) -20;		
